...
/** Fetches the Discord authorize URL from the backend (with a signed,
 * short-lived "state" baked in) and returns it so the browser can
 * navigate straight to discord.com — the user never visits the
 * onrender.com backend as a page. */
export async function fetchDiscordLoginUrl(): Promise<string> {
  const res = await fetch(`${API_BASE}/api/auth/discord/url`, {
    headers: { Accept: "application/json" },
  });
  if (!res.ok) {
    throw new ApiError(`/api/auth/discord/url failed with ${res.status}`, res.status);
  }
